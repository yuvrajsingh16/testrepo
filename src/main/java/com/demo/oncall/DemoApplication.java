...
public class DemoApplication {
    ...
    public Map<String, Object> processOrder(
        ...
        try {
            ...
            log.warn("Order rejected due to invalid user profile data. userId={}, product={}, quantity={}, reason={}",
                userId, product, quantity, e.getMessage());

            NewRelic.noticeError(e, Map.of(
                "userId", userId,
                "product", product,
                "errorType", "InvalidUserProfile",
            ...
            return Map.of(
                "status", "error",
                "error", "Invalid user profile: cannot generate shipping label for user " + userId,
                "detail", e.getMessage(),
                "timestamp", LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)
            );
    ...
    private String formatShippingLabel(UserProfile user) {
        if (user == null) {
            throw new IllegalArgumentException("User profile is required to format a shipping label");
        }
