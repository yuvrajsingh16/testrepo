...
public class DemoApplication {
    ...
    public Map<String, Object> processOrder(
        ...
        try {
            ...
            // FIX: Fail fast on missing required address data (prevents NPE in downstream formatting).
            if (user.address == null || user.address.isBlank()) {
                log.error("Invalid order: missing shipping address for userId={} product={} qty={}", userId, product, quantity);
                return Map.of(
                    "status", "error",
                    "error", "Shipping address is required",
                    "detail", "User profile is missing a shipping address; update profile and retry.",
